...
## How to use it?
----------
This toolbox contains a model (named DAHGMN), a set of data processing support functions (under utils), and a demo.py file.
The demo.py script provides an example experiment using the Houston dataset. You can simply run demo.py and adjust different parameters to obtain results.

In addition, we provide a well-structured framework that supports single-modal and multi-modal data inputs, allows sample splitting by ratio or fixed quantity, performs validation and testing, and includes visualization capabilities.
You only need to modify the network architecture to directly run new experiments.
