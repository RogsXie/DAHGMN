...
parser = argparse.ArgumentParser("GCNMamba")
parser.add_argument('--seed', type=int, default=0, help='Random seed for reproducibility')
parser.add_argument('--test_freq', type=int, default=10, help='Evaluation frequency in epochs')
parser.add_argument('--epoches', type=int, default=500, help='Total training epochs')
...
parser.add_argument('--dataset', default='Houston', choices=['Muufl', 'Trento', 'Houston'], help='Dataset selection')
parser.add_argument('--num_classes', type=int, default=15, choices=[11, 6, 15], help='Number of classes')
...
def create_dataloader():
    ...
    # Data1, Data2, gt, _, test_labels = load_data(args.dataset)
    ...
    # train_labels, valid_labels= split_train_test_labels(
    ...
    # train_labels, _ = loadtrandte_data(args.dataset)
    ...
    # ValidPatch1,ValidPatch2,ValidLabel= traintwo_patch(
    #     Data1, Data2, args.patches1, pad_width, valid_labels, args.more0
    # )
    ...
    # valid_dataset=Data.TensorDataset(ValidPatch1,ValidPatch2,ValidLabel)
    test_dataset = Data.TensorDataset(TestPatch1, TestPatch2, TestLabel)

    print(f"训练集样本数量: {len(train_dataset)}")
    # print(f"验证集集样本数量: {len(valid_dataset)}")
...
def evaluate(model, data_loader, criterion, device):
    """Return (avg_loss, accuracy_percent) on given loader."""
    model.eval()
    total, correct = 0, 0
    running_loss = 0.0
    with torch.no_grad():
...
def train(model, optimizer, criterion, scheduler, train_loader, valid_loader, device):
    ...
    for epoch in range(args.epoches):
        ...
        avg_train_loss = training_loss / max(1, len(train_loader))
